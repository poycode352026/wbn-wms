...
const form = useForm({ name: '', employee_id: '', email: '', password: '', role: 'operator', department_id: null, is_active: true })
...
function openEdit(user) {
    ...
    Object.assign(form, { name: user.name, employee_id: user.employee_id ?? '', email: user.email ?? '',
        password: '', role: user.role, department_id: user.department_id, is_active: user.is_active })
...
const ROLES = ['super_admin','admin_dept','manager_dept','warehouse_manager','supervisor','operator']
const RC = {
    super_admin:       { bg:'rgba(249,115,22,.15)',  color:'#fb923c', lbl:'SUPER ADMIN' },
    admin_dept:        { bg:'rgba(59,130,246,.15)',  color:'#60a5fa', lbl:'ADMIN DEPT'  },
    manager_dept:      { bg:'rgba(139,92,246,.15)',  color:'#a78bfa', lbl:'MANAGER'     },
    warehouse_manager: { bg:'rgba(249,115,22,.15)',  color:'#fb923c', lbl:'WH MANAGER'  },
    supervisor:        { bg:'rgba(234,179,8,.15)',   color:'#facc15', lbl:'SUPERVISOR'  },
    operator:          { bg:'rgba(100,116,139,.12)', color:'#94a3b8', lbl:'STAFF'       },
}
const AC = [
      ...
      <div v-if="isOpen" class="mo" @click.self="closeModal">
        <div class="modal">
          ...
          <div class="mb2">
            ...
            <!-- Department -->
            <div class="fg full">
              <label class="fl">{{ $t('um.department') }}</label>
              <select v-model="form.department_id" class="fi fis">
                <option :value="null">{{ $t('um.noDept') }}</option>
                <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
              </select>
            </div>
          </div>
...
/* modal — always dark regardless of app theme */
.mo{
  position:fixed;inset:0;z-index:50;background:rgba(0,0,0,.72);backdrop-filter:blur(5px);
  display:flex;align-items:center;justify-content:center;padding:24px;
  /* force dark CSS vars inside modal */
  --surface:#0d1117;--surface-2:#111827;--surface-3:#1e2535;
  --border:#1e2535;--border-2:#2a3550;
  --fg:#e6edf7;--fg-2:#94a3b8;--fg-dim:#64748b;
  --hover:rgba(59,130,246,.08);--emerald:#10b981;--rose:#ef4444;--blue-500:#3b82f6;
