...
onMounted(() => {
    ;(props.grq?.items ?? []).forEach(item => {
        actualQtys.value[item.id]   = Number(item.qty_in_base_uom ?? 0)
        itemStatuses.value[item.id] = item.item_status === 'rejected' ? 'rejected' : 'ready'
...
function setStatus(itemId, status) {
    ...
    if (status === 'rejected') {
        ...
        if (parseFloat(actualQtys.value[itemId] ?? 0) === 0) {
            const item = (props.grq?.items ?? []).find(i => i.id === itemId)
            actualQtys.value[itemId] = Number(item?.qty_in_base_uom ?? 0)
        }
    }
}

const rejectedCount = computed(() =>
    (props.grq?.items ?? []).filter(i => itemStatuses.value[i.id] === 'rejected').length
)
...
const canSubmit = computed(() =>
    ...
    (props.grq?.items ?? []).length > 0 &&
    (props.grq?.items ?? []).every(item => {
...
function doStartPicking() {
    startForm.post(route('operator.start-picking-grq', props.grq.id))
}

function doSubmitPickup() {
    const items = (props.grq?.items ?? []).map(item => ({
        id:         item.id,
    ...
    submitForm
        .transform(() => ({ items }))
        .post(route('operator.submit-pickup-grq', props.grq.id), { preserveScroll: true })
}
...
async function doConfirmPickup() {
    ...
    try {
        ...
        router.post(route('operator.confirm-pickup-grq', props.grq.id), fd, {
            forceFormData: true,
...
<template>
<OperatorLayout>
  <template #title>{{ grq?.grq_number }} · {{ grq?.department?.name }}</template>
  ...
  <div class="pk-wrap">
    ...
    <div class="pk-topbar">
      <div class="pk-topbar-left">
        ...
        <div>
          <div class="pk-gi-number">{{ grq?.grq_number }}</div>
          <div class="pk-gi-dept">{{ grq?.department?.name ?? '—' }}</div>
        </div>
      </div>
      <!-- Status badge -->
      <span class="pk-status-badge" :class="`pk-status-${grq?.status}`">
        {{ { assigned: t('operator.statusAssigned'), in_picking: t('operator.statusPicking'), ready_to_pickup: t('operator.statusReady'), completed: t('operator.statusDone') }[grq?.status] ?? grq?.status }}
      </span>
    </div>
    ...
    <!-- ════════════════════════════════════════════════════════════ -->
    <div v-if="grq?.status === 'assigned'" class="pk-state-card pk-state-assigned">
      ...
      <div class="pk-preview-items">
        <div v-for="item in (grq?.items ?? [])" :key="item.id" class="pk-preview-row">
          <span class="pk-preview-name">{{ itemName(item.variant) }}</span>
          <span class="pk-preview-qty">{{ fmtQty(item.qty_in_base_uom) }} {{ item.variant?.item?.base_uom }}</span>
        </div>
      </div>
    ...
    <!-- ════════════════════════════════════════════════════════════ -->
    <!-- STATE 2: IN_PICKING — Actual Qty + Status + Notes          -->
    <!-- ════════════════════════════════════════════════════════════ -->
    <template v-else-if="grq?.status === 'in_picking'">

      <!-- Summary bar -->
      <div class="pk-pick-summary">
        <span class="pk-pick-total">{{ grq?.items?.length ?? 0 }} item</span>
          ...
          <div class="pk-item-body">
            ...
            <div class="pk-item-header">
              ...
              <span class="pk-item-req-qty">
                Req {{ fmtQty(item.qty_in_base_uom) }} {{ item.variant?.item?.base_uom }}
              </span>
            </div>
            ...
            <div class="pk-issue-row">
              ...
              <div class="pk-issue-qty-wrap">
                ...
                <input
                  ...
                  inputmode="numeric"
                />
                <span class="pk-qty-uom">{{ item.variant?.item?.base_uom }}</span>
              </div>
    ...
    <!-- ════════════════════════════════════════════════════════════ -->
    <div v-else-if="grq?.status === 'ready_to_pickup'" class="pk-state-card pk-state-ready">
      ...
      <div class="pk-ready-items">
        <div v-for="item in (grq?.items ?? [])" :key="item.id" class="pk-ready-row">
          <span class="pk-ready-name">{{ itemName(item.variant) }}</span>
          <span class="pk-ready-qty">
            {{ fmtQty(item.actual_qty ?? item.qty_in_base_uom) }} {{ item.variant?.item?.base_uom }}
          </span>
        </div>
      </div>
...
.pk-status-assigned       { background:rgba(59,130,246,.14);  color:#3b82f6; }
...
.pk-state-assigned { border-color: rgba(59,130,246,.4); background: rgba(59,130,246,.03); }
...
.pk-preview-qty  { color: #3b82f6; font-weight: 700; white-space: nowrap; }
...
.pk-btn-start    { background: #3b82f6; color: #fff; }
.pk-btn-submit   { background: #3b82f6; color: #fff; width: 100%; max-width: 100%; }
...
.pk-qty-input:focus { border-color: #3b82f6; }
...
.pk-notes-ta:focus    { border-color: #3b82f6; }
